...
public class DeleteUserByIdCommand implements ActionCommand {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            User user = (User) userManagementAPI.getUserById(userId);
            request.setAttribute("user", user);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }

        // Pass user object to the deletion confirmation form
        page = ConfigurationManager.getProperty("path.page.deleteUserForm");
